...
-   카카오톡 로그인 연동
-   firebase 이용해서 회원가입,로그인,로그아웃 구현
...
#### 4) firebase 이용해서 회원가입,로그인,로그아웃 구현

 카카오톡 로그인 구현 했지만 하나만 있기에는 웹사이트 같다는 생각이 안들어서 고민 끝에 firebase로 이용해서 데이터 베이스를 만들고 회원가입,로그인,로그아웃을 구현 했습니다.
