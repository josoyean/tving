...
const ProgramInfor = () => {
  ...
  useEffect(() => {
    ...
    console.log(userInfo.select.genre);
  }, []);
  ...
  const programCilck = (e, item) => {
    e.stopPropagation();
    navigator("/program", { state: { select: item, all: userInfo.all } });
  };
  return (
    <div>
      <Nav
          ...
          <div className="main-infor">
            ...
            <div className="infor">
              {userInfo.select.channel && (
                <span>{userInfo.select.channel}</span>
              )}
              {userInfo.select.genre && <span>{userInfo.select.genre}</span>}
              {userInfo.select.type && <span>{userInfo.select.type}</span>}
            </div>
